...
- Weather data (five variants, from **weatheryeardata**) is staged
...
- Script B keeps all five of `WEATHERYEAR`'s `.inc`-producing variants -
  `HourlyDispatch/{raw,scaled_long_term}` (the two the WY job scripts
  actually read, see above) *and* `CapDev/{raw,scaled_long_term,
  scaled_full_year}` (investment-resolution timesteps, unread by anything
  in this pipeline today) - rather than discarding the ones this decision's
  fixed-investment scope doesn't currently need. Cheap to keep (it's
  already been generated by the time script B runs) and avoids losing data
  a future weather-year-aware investment run would want, without this ADR
  having to speculate about what that run would look like.
