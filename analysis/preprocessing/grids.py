...
def electricity_transmission():
    ...
    suffixes = [
        "\n$label unconstrained\n;\n* Ensure symmetry\nXMAXINV(IRRRE,IRRRI)=MAX(XMAXINV(IRRRI,IRRRE),XMAXINV(IRRRE,IRRRI));\n$ifi %tyndp2039%==yes XMAXINV(IRRRE,IRRRI)$(XMAXINV(IRRRE,IRRRI) EQ 0 AND XINVCOST('2050',IRRRE,IRRRI))=XKFX('2050',IRREE,IRRRI);",
...
def hydrogen_transmission():
    ...
    incfile = IncFile(
        ...
        suffix="\n$label unconstrained\n;\n* Ensure symmetry\nXH2MAXINV(IRRRE,IRRRI)=MAX(XH2MAXINV(IRRRI,IRRRE),XH2MAXINV(IRRRE,IRRRI));\n$ifi %tyndp2039%==yes XH2MAXINV(IRRRE,IRRRI)$(XH2MAXINV(IRRRE,IRRRI) EQ 0 AND XH2INVCOST('2050',IRRRE,IRRRI))=XH2KFX('2050',IRREE,IRRRI);",
        path="analysis/Balmorel/base/data",
    )
    df = df.astype(object)
    idx = df < 1e-5
